...
(function() {
    ...
    window.alert = function(message) {
        if (output) {
            output.innerHTML += `<span style="color: gray;">Вывод:</span> ${message}</br> <br>`;
        }
...
(function() {
    ...
    window.prompt = function(message) {
        ...
        if (output) {
            ...
            output.innerHTML += `<span style="color: gray;">Ввод:</span> ${message} = ${displayValue}</br> <br>`;
        }
...
function ShowToast(title, content, timeout = 3000) {
    ...
    const removalTimeout = setTimeout(closeToast, timeout);
